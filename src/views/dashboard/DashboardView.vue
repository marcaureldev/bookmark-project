...
import ReadingProgressCard from "../../components/dashboard/ReadingProgressCard.vue";
...
const activeReadingTags = computed(() => {
  if (bookmarks.value.length === 0) return [];

  const readingBookmarks = bookmarks.value.filter(
    (bookmark) => bookmark.status === "reading"
  );
  ...
  readingBookmarks.forEach((bookmark) => {
    if (bookmark.category) {
      const tags = bookmark.category.includes(",")
        ? bookmark.category
            .split(",")
            .map((tag: string) => tag.trim())
  ...
  <div class="flex-1 flex flex-col">
    ...
    >
      ...
      <StatCard
        ...
        color="purple"
        :percentage="
          stats.total > 0 ? Math.round((stats.reading / stats.total) * 100) : 0
        "
      />
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      <div v-if="!isLoading && !error" class="mb-8">
          ...
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-sm text-gray-600">
                Bookmarks ajoutés cette semaine
              </p>
            </div>
            <div
      ...
      <ReadingProgressCard
        :reading-count="stats.reading"
        :completed-count="stats.read"
        :total-count="stats.total"
        :active-tags="activeReadingTags"
      />
    </div>
